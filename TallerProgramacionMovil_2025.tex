...
\begin{itemize}
\item Contiene un \textbf{hilo secundario} (a menudo un \texttt{GameThread} o similar) que ejecuta el bucle de juego para \textbf{actualizar la lógica} (\texttt{update()}) y \textbf{dibujar} (\texttt{draw()}) en un ciclo constante.
\item Esto asegura que las animaciones y la física se ejecuten fluidamente sin bloquear el Hilo Principal (UI).
\end{itemize}
